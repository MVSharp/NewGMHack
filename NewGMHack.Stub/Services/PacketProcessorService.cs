...
//1E 00 F0 03 FA 07 46 EF 00 00 (personId[the guy changeit]) 4F 14 00 00 00 00 00 00 04 00 45 C7 00 00 30 75 45 24 14 00
    ...
    public void SendF5(IntPtr socket)
    {
        ...
        ReadOnlySpan<byte> msg5 = [0x0A, 0x00, 0xF0, 0x03, 0x89, 0x09, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00];
        ReadOnlySpan<byte> msg6 = [0x06, 0x00, 0xF0, 0x03, 0x1A, 0x09, 0x00, 0x00, 0x00, 0x00];
        #region Captain
        ReadOnlySpan<byte> msg7 =
        [
    ...
    private void SendZoneActiviate(IntPtr socket)
    {
        ...
        ReadOnlySpan<byte> zone4 =
        [
            0x17, 0x00, 0xF0, 0x03, 0x6A, 0x09, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFD, 0x8E, 0x47, 0x45,
            0xD7, 0x77, 0xD9, 0xC3, 0x7A, 0x68, 0x31, 0x00, 0x02
        ];
        ReadOnlySpan<byte> zone5 =
        [
            0x17, 0x00, 0xF0, 0x03, 0x6A, 0x09, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xC0, 0xBE, 0xE7, 0x44,
            0x0A, 0x40, 0xC4, 0x43, 0x79, 0x68, 0x31, 0x00, 0x01
        ];
        ReadOnlySpan<byte> zone6 =
        [
            0x17, 0x00, 0xF0, 0x03, 0x6A, 0x09, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x69, 0x55, 0x17, 0xC5,
            0x99, 0x50, 0x18, 0xC4, 0x7B, 0x68, 0x31, 0x00, 0x03
        ];
        ...
        _winsockHookManager.SendPacket(socket, zone4);

        _winsockHookManager.SendPacket(socket, zone5);

        _winsockHookManager.SendPacket(socket, zone6);
    }
