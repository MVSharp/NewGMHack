...
# endif
        ...
        /// </summary>
        public bool HasTrampoline => m_Trampoline != null;
                ...
                functionName)
        {
            ...
            memoryBlock = IntPtr.Add(memoryBlock, 1);
#if X64
		//push rax
...
#endif
            if ((m_Trampoline = Trampoline.Create(originalFunc, m_pHookJump + sizeof(JMP_ABS))) != null)
            {
                ...
                NativeMethods.VirtualProtect(originalFunc, (nint)sizeof(JMP_REL), MemoryProtection.ExecuteReadWrite,
                                             &oldProtect);
                ...
                *(JMP_REL*)originalFunc = new JMP_REL(MemoryBlock + 1, originalFunc);
                ...
                NativeMethods.VirtualProtect(originalFunc, (nint)sizeof(JMP_REL), oldProtect, &oldProtect);
            }
            else
            {
        ...
        public virtual bool InstallHook()
        {
            ...
            lock (this)
            {
                ...
                else
                {
                    ...
                    MemoryProtection oldProtect;
                    if (!NativeMethods.VirtualProtect(OriginalFunction, (nint)sizeof(ulong),
                                                      MemoryProtection.ExecuteReadWrite, &oldProtect))
                        return false;

                    *(JMP_REL*)OriginalFunction = new JMP_REL(m_pHookJump, OriginalFunction);

                    if (!NativeMethods.VirtualProtect(OriginalFunction, (nint)sizeof(ulong), oldProtect, &oldProtect))
                        return false;
                }
...
# endif
                    functionName = null)
        {
            if (nativeFunctionEntryPoint == IntPtr.Zero || hookFunction == IntPtr.Zero) return null;

            NativeMethods.VirtualQuery(nativeFunctionEntryPoint, out var mbi, sizeof(MemoryBasicInformation));

            if (!(mbi.Protect is MemoryProtection.Execute          ||
                  mbi.Protect is MemoryProtection.ExecuteRead      ||
                  mbi.Protect is MemoryProtection.ExecuteReadWrite ||
                  mbi.Protect is MemoryProtection.ExecuteWriteCopy))
                return null;
        ...
        unsafe public static
            ...
            Create(
                ...
                bool         installAfterCreate = true)
        {
            if (hookFunction == IntPtr.Zero) return null;

            var moduleName = export.Module.FileName ?? export.Module.ModuleName;
            if ((moduleName is null) || !NativeLibrary.TryLoad(moduleName, out var hModule))
                return null;

            string exportFuncName;
            if (export.FunctionName is string expName &&
                NativeLibrary.TryGetExport(hModule, export.FunctionName, out nint originalFunc))
                exportFuncName = expName;
            else if ((originalFunc = NativeMethods.GetProcAddress(hModule, export.Ordinal)) != IntPtr.Zero)
        ...
        public virtual void Dispose()
        {
            if (!IsDisposed)
            {
                if (HasTrampoline)
                {
                    ...
                    {
                        MemoryProtection oldProtect;
                        NativeMethods.VirtualProtect(m_Trampoline.TargetAddress, (nint)sizeof(ulong),
                                                     MemoryProtection.ExecuteReadWrite, &oldProtect);
                        *(ulong*)m_Trampoline.TargetAddress = m_OriginalEpBytes;
                        //Restore IAT's protection
                        NativeMethods.VirtualProtect(m_Trampoline.TargetAddress, (nint)sizeof(ulong), oldProtect,
                                                     &oldProtect);
                    }
                }
                else
