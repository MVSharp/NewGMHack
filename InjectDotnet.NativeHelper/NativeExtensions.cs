...
# endif
            ...
            where TDelegate : Delegate
        {
            ...
            return BreakpointHook.Create(export.Module.BaseAddress.Add((IntPtr)export.FunctionRVA.Value), hookPointer,
                                         thread.Id, installAfterCreate);
        }
                                 ...
                                 procModule)
        {
            ...
            using (var peFile =
                   new BinaryReader(File.Open(procModule.FileName, FileMode.Open, FileAccess.Read, FileShare.Read)))
            {
                    ...
                    while (*(IntPtr*)(hModule + iatEntryRVA) != IntPtr.Zero)
                    {
                        ...
                        if (list.FirstOrDefault(i
                                                    => i.Library == import.Library &&
                                                       ((i.FunctionName != null &&
                                                         i.FunctionName == import.FunctionName) ||
                                                        (i.Ordinal != null && i.Ordinal == import.Ordinal))) is
                            NativeImport existing)
                        {
        ...
        internal static string RemoveDllExtension(this string libName)
            => libName.EndsWith(".dll", StringComparison.OrdinalIgnoreCase)
                ? libName.Remove(libName.Length - 4, 4)
                : libName;
