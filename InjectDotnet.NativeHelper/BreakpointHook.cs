    ...
    public class BreakpointHook : INativeHook
    {
            ...
            Breakpoint breakpoint)
        {
            ...
            if (Debugger.IsAttached)
                throw new
                    InvalidOperationException("Cannot use hardware breakpoint hooks while debugging the hooked process.");
            ...
            bool installAfterCreate = true)
        {
            ...
            var hwBp
                ...
                ?.FirstOrDefault(b => b.Address == IntPtr.Zero);

            if (hwBp == null) return null;
            ...
            var trampoline = Trampoline.Create(nativeFunctionEntryPoint, memoryBlock);
            if (trampoline == null)
                return null;
        ...
        private static unsafe int VectoredExceptionHandler(ref ExceptionPointers exceptionInfo)
        {
            ...
            var threadBP = ThreadBreakpoints.GetThreadBreakpoints(currentThread);
            if (threadBP == null)
                return EXCEPTION_CONTINUE_SEARCH;
        ...
        public void Dispose()
        {
            if (!IsDisposed)
            {
                ...
                Breakpoint.Address   = IntPtr.Zero;
