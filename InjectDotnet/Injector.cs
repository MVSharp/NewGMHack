...
using InjectDotnet.Debug.DebugEventArgs;
...
public static class Injector
{
    ...
    public static int? Inject<T>(this Process target,                     string runtimeconfig, string dllToInject,
                                 string       asssemblyQualifiedTypeName, string method,        T      methodArgument,
    ...
    public static void InjectStartup<T>(this Debug.Debugger debugger, string runtimeconfig, string dllToInject,
                                        string              asssemblyQualifiedTypeName, string method, T methodArgument,
                                        bool                detatchAfterInjected = true)
        where T : struct
    {
        ...
        debugger.Breakpoint += (s, e) =>
        {
            if (e.Type is BreakType.EntryPoint)
            {
                ...
                var loader = CoreLoader.Create(process, runtimeconfig, dllToInject, asssemblyQualifiedTypeName, method,
                                               argBts, true);
    ...
    /// <param name="waitForReturn">Whether to wait for <paramref name="method"/> to return</param>
    /// <returns>If <paramref name="waitForReturn"/> is true, the value returned by <paramref name="method"/>. Otherwise null.</returns>
    public static int? Inject(this Process target,                     string runtimeconfig, string dllToInject,
                              string       asssemblyQualifiedTypeName, string method,        string argument,
                              bool         waitForReturn = true)
    {
    ...
    public static void InjectStartup(this Debug.Debugger debugger, string runtimeconfig, string dllToInject,
                                     string asssemblyQualifiedTypeName, string method, string methodArgument,
                                     bool detatchAfterInjected = true)
    {
        if (debugger.HasHitEntryPoint)
            throw new InvalidOperationException("Cannot inject after execution has passed the debugee's entry point.");
    ...
    /// <returns>If <paramref name="waitForReturn"/> is true, the value returned by <paramref name="method"/>. Otherwise null.</returns>
    public static int? Inject(this Process target,                     string runtimeconfig, string dllToInject,
                              string       asssemblyQualifiedTypeName, string method, ReadOnlySpan<byte> methodArgument,
                              bool         waitForReturn = true)
    {
        var loader = CoreLoader.Create(target, runtimeconfig, dllToInject, asssemblyQualifiedTypeName, method,
                                       methodArgument, false);
    ...
    public static int? Inject(this Process target, string dotnetFrameworkDll, string typeName, string methodName,
                              string       methodArgument, bool waitForReturn = true)
    {
    ...
    /// <param name="methodName">The name of the static method to invoke. Must have signature <see cref="int"/>(<see cref="string"/>)</param>
    /// <param name="methodArgument">The string parameter to pass to <paramref name="methodName"/>.</param>
    public static void InjectStartup(this Debug.Debugger debugger, string dotnetFrameworkDll, string typeName,
                                     string methodName, string methodArgument, bool detatchAfterInjected = true)
    {
        if (debugger.HasHitEntryPoint)
            throw new InvalidOperationException("Cannot inject after execution has passed the debugee's entry point.");
        ...
        debugger.Breakpoint += (s, e) =>
        {
            if (e.Type is BreakType.EntryPoint)
            {
                ...
                var loader =
                    FrameworkLoader.Create(process, dotnetFrameworkDll, typeName, methodName, methodArgument, true);
                e.Context.InstructionPointer = loader.Loader;
#if X64
				e.Context.Rcx = loader.Parameter;
#elif X86
